...
SECURITY_GOALS = {
    "ЦБ-1": "Монитор безопасности должен разрешать передачу в исполнительный компонент только команд, соответствующих допустимым состояниям светофора.",
    "ЦБ-2": "Система управления должна изменять состояние светофора только через монитор безопасности, без прямого доступа к исполнительному компоненту.",  # Сформулируйте цель безопасности
    "ЦБ-3": "Монитор безопасности должен блокировать и регистрировать команды, приводящие к физически опасным комбинациям сигналов.",  # Сформулируйте цель безопасности
}

SECURITY_ASSUMPTIONS = {
    "ПБ-1": "Исполнительный компонент LightsGPIO и монитор безопасности считаются доверенными и не скомпрометированными.",  # Сформулируйте предположение безопасности
    "ПБ-2": "Нарушитель не имеет прямого физического доступа к линиям управления сигналами светофора и не может обходить монитор безопасности.",  # Сформулируйте предположение безопасности
    "ПБ-3": "Физический доступ нарушителя к линиям управления светодиодами и доверенной вычислительной базе отсутствует.",
}
...
class Monitor:
    ...
    Проверяет события и передаёт только разрешённые команды."""
